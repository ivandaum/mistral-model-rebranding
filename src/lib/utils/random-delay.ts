...
// allow to set the start, the step delay between letter
// and a max (to avoid the animation to take too much time)
/**
 *
 * @param step : the step (in ms) between each elements.
 * 							 Usually calculate with `i * my_step` to allow every element to
 * 							 be animated after its previous sibling
 * @param maxDelay : the max delay it can be for each element
