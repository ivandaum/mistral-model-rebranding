<!-- This is coming straight from Mistral website -->

<header class="absolute z-999 flex w-full items-center">
	<div
		class="bg-background pointer-events-none absolute top-0 left-0 z-0 w-full dark:text-white"
		style="height:100%"
	></div>
	<div
		class="md:px-auto relative z-10 m-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-6 md:container"
	>
		<div class="flex items-center gap-8">
			<a
				rel="home"
				aria-label="Home"
				class="relative z-10 flex size-10 items-center justify-center"
				href="/"
				><img
					alt="Mistral AI Logo"
					width="100"
					height="100"
					decoding="async"
					data-nimg="1"
					class="object-contain"
					style="color:transparent"
					src="https://mistral.ai/_next/image?url=%2Fimg%2Fmistral-ai-logo.svg&w=256&q=75&dpl=0dd24beb3955b26c79ed2326818d15f93bb8e9a88eed4b1d249a29fb095b2e15363961353638643532346132396134313263383164343764"
				/>
				<div
					class="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden transition-all duration-500 ease-in-out"
					style="clip-path:inset(100% 0 0 0);-webkit-clip-path:inset(100% 0 0 0)"
				>
					<img
						alt="Mistral AI Logo White"
						width="100"
						height="100"
						decoding="async"
						data-nimg="1"
						class="absolute object-contain"
						style="color:transparent"
						srcset="/_next/image?url=%2Fimg%2Fmistral-ai-logo-white.svg&amp;w=128&amp;q=75&amp;dpl=0dd24beb3955b26c79ed2326818d15f93bb8e9a88eed4b1d249a29fb095b2e15363961353638643532346132396134313263383164343764 1x, /_next/image?url=%2Fimg%2Fmistral-ai-logo-white.svg&amp;w=256&amp;q=75&amp;dpl=0dd24beb3955b26c79ed2326818d15f93bb8e9a88eed4b1d249a29fb095b2e15363961353638643532346132396134313263383164343764 2x"
						src="/_next/image?url=%2Fimg%2Fmistral-ai-logo-white.svg&amp;w=256&amp;q=75&amp;dpl=0dd24beb3955b26c79ed2326818d15f93bb8e9a88eed4b1d249a29fb095b2e15363961353638643532346132396134313263383164343764"
					/>
				</div>
				<div class="pointer-events-none absolute inset-0"></div></a
			>
			<nav class="hidden items-center gap-6 lg:flex" aria-label="Main navigation">
				<button
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					>Products
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></button
				><a
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					href="/solutions"
					>Solutions
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></a
				><a
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					href="/models"
					>Research
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></a
				><a
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					href="/news"
					>Blog
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></a
				><a
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					href="/customers"
					>Customers
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></a
				><button
					class="group flex items-center gap-2 py-4 text-sm text-black opacity-80 transition-opacity duration-500 hover:opacity-100 dark:text-white"
					role="menuitem"
					aria-haspopup="menu"
					aria-expanded="false"
					>Company
					<div class="relative h-3 overflow-hidden text-mistral-orange dark:text-mistral-orange">
						<div class="flex h-6 flex-col transition-transform duration-200">
							<svg
								class="size-3 shrink-0 rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							><svg
								class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
								width="24"
								viewBox="0 0 9 13"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
								><path
									d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
									fill="currentColor"
								></path><path
									d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
									fill="currentColor"
								></path><path
									d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
									fill="currentColor"
								></path><path
									d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
									fill="currentColor"
								></path><path
									d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
									fill="currentColor"
								></path></svg
							>
						</div>
					</div></button
				>
			</nav>
		</div>
		<div class="flex items-center justify-end gap-2">
			<a
				target="_self"
				class="group text-mistral-black bg-mistral-beige-deep/50 hover:bg-mistral-beige-deep hidden items-center gap-2 truncate px-4 py-2 text-sm md:flex dark:text-white"
				href="/contact"
				>Contact Sales<svg
					class="ml-2 inline-block size-3 text-mistral-orange transition-transform group-hover:translate-x-0.5"
					width="24"
					viewBox="0 0 9 13"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
					><path
						d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
						fill="currentColor"
					></path><path
						d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
						fill="currentColor"
					></path><path
						d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
						fill="currentColor"
					></path><path
						d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
						fill="currentColor"
					></path><path
						d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
						fill="currentColor"
					></path></svg
				></a
			>
			<div class="relative hidden md:block">
				<div
					class="group dark:bg-mistral-beige-deep/50 dark:hover:bg-mistral-beige-deep flex items-center bg-black text-sm text-white"
				>
					<a
						target="_blank"
						rel="noopener"
						class="truncate px-4 py-2 transition-colors hover:bg-white/10"
						href="https://console.mistral.ai?utm_source=website&amp;utm_medium=header_cta"
						>Try Studio</a
					><button
						aria-haspopup="menu"
						aria-expanded="false"
						class="flex size-9 items-center justify-center border-l border-white/30 text-white hover:bg-white/10 dark:border-white/10"
						><div class="relative h-3 overflow-hidden text-mistral-orange dark:text-white">
							<div class="flex h-6 flex-col transition-transform duration-200">
								<svg
									class="size-3 shrink-0 rotate-90 transition-transform duration-300"
									width="24"
									viewBox="0 0 9 13"
									fill="none"
									xmlns="http://www.w3.org/2000/svg"
									><path
										d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
										fill="currentColor"
									></path><path
										d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
										fill="currentColor"
									></path><path
										d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
										fill="currentColor"
									></path><path
										d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
										fill="currentColor"
									></path><path
										d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
										fill="currentColor"
									></path></svg
								><svg
									class="size-3 shrink-0 -rotate-90 transition-transform duration-300"
									width="24"
									viewBox="0 0 9 13"
									fill="none"
									xmlns="http://www.w3.org/2000/svg"
									><path
										d="M8.5 7.86859L8.5 5.30859L5.94 5.30859L5.94 7.86859L8.5 7.86859Z"
										fill="currentColor"
									></path><path
										d="M5.9375 5.29828L5.9375 2.73828L3.3775 2.73828L3.3775 5.29828L5.9375 5.29828Z"
										fill="currentColor"
									></path><path
										d="M3.37891 2.73969L3.37891 0.179688L0.818907 0.179687L0.818907 2.73969L3.37891 2.73969Z"
										fill="currentColor"
									></path><path
										d="M5.9375 10.4311L5.9375 7.87109L3.3775 7.87109L3.3775 10.4311L5.9375 10.4311Z"
										fill="currentColor"
									></path><path
										d="M3.37891 12.9975L3.37891 10.4375L0.818907 10.4375L0.818907 12.9975L3.37891 12.9975Z"
										fill="currentColor"
									></path></svg
								>
							</div>
						</div></button
					>
				</div>
			</div>
			<button
				class="text-mistral-black cursor-pointer md:px-2 lg:hidden dark:text-white"
				aria-label="Open menu"
				><svg width="24" viewBox="0 0 75 63" fill="none" xmlns="http://www.w3.org/2000/svg"
					><path opacity="0.5" d="M49.76 49.7598H0V62.1998H49.76V49.7598Z" fill="currentColor"
					></path><path
						opacity="0.7"
						d="M74.64 24.8799H0V37.3199H74.64V24.8799Z"
						fill="currentColor"
					></path><path d="M74.64 0H0V12.44H74.64V0Z" fill="currentColor"></path></svg
				></button
			>
		</div>
	</div>
</header>
